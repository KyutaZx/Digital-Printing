    ...
    <header class="w-full max-w-5xl rounded-full pointer-events-auto transition-all duration-300 relative"
        ...
        <div class="flex items-center justify-between h-16 px-4 lg:px-6">
            ...
            <div class="hidden md:flex items-center gap-3">
                @if(session('user'))
                    <!-- Cart Icon & Count -->
                    <div x-data="{ count: 0 }" x-init="fetch('{{ url('/api/cart') }}', {headers: {'Authorization': 'Bearer {{ session('token') }}'}}).then(r => r.json()).then(d => { if(d.items) count = d.items.reduce((acc, item) => acc + item.quantity, 0) }).catch(() => {})">
