                    ...
                    <div class="flex items-start gap-4">
                        <div class="w-16 h-16 bg-white rounded-xl border border-slate-200 overflow-hidden shrink-0">
                            @if(!empty($item['product_image']))
                                <img src="{{ url('/api-proxy/' . ltrim($item['product_image'] ?? '', '/')) }}" class="w-full h-full object-cover">
                            @else
                    ...
                    @if($hasDesign && !$canReupload)
                        <div class="flex items-start gap-4">
                            ...
                            @if(in_array($ext, ['jpg','jpeg','png']))
                                <a href="{{ url('/api-proxy/' . ltrim($latestDesign['file_path'] ?? '', '/')) }}" target="_blank"
                                   class="shrink-0 block w-20 h-20 rounded-xl overflow-hidden border border-slate-200 bg-slate-50 hover:opacity-90 transition-opacity">
                                    <img src="{{ url('/api-proxy/' . ltrim($latestDesign['file_path'] ?? '', '/')) }}" alt="Desain" class="w-full h-full object-contain p-1">
                                </a>
                            @else
                                <a href="{{ url('/api-proxy/' . ltrim($latestDesign['file_path'] ?? '', '/')) }}" target="_blank"
