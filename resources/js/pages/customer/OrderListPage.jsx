...
export default function OrderListPage() {
  ...
  const getStatusBadge = (status) => {
    switch(status) {
      ...
      case 'reviewing': return <Badge variant="warning">Menunggu Review Desain</Badge>;
  ...
  const filteredOrders = userOrders.filter(order => {
    ...
    if (activeFilter === 'processing') return ['verifying', 'reviewing', 'production'].includes(order.status);
              ...
              {filteredOrders.map((order) => (
                ...
                >
                  ...
                  <div className="bg-slate-50 border-b border-slate-100 px-6 py-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div>
                      <div className="text-xs text-slate-500 mb-1">{formatDate(order.createdAt)}</div>
