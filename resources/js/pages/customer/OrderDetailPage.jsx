...
export default function OrderDetailPage() {
  ...
  const steps = [
    { id: 1, label: 'Pesanan Dibuat', status: ['pending', 'verifying', 'reviewing', 'production', 'completed'].includes(order.status) },
    { id: 2, label: 'Pembayaran', status: ['verifying', 'reviewing', 'production', 'completed'].includes(order.status) },
    { id: 3, label: 'Review Desain', status: ['reviewing', 'production', 'completed'].includes(order.status) },
    { id: 4, label: 'Produksi', status: ['production', 'completed'].includes(order.status) },
    { id: 5, label: 'Selesai', status: ['completed'].includes(order.status) },
  ];
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">Tanggal Order: {formatDate(order.createdAt)}</p>
          </div>
        </div>
        ...
        {order.status === 'pending' && !isExpired && (
          <FadeIn className="bg-orange-50 border border-orange-200 rounded-2xl p-6 mb-8 flex flex-col sm:flex-row items-center justify-between gap-6 shadow-sm">
            <div className="flex-grow">
              ...
              <p className="text-sm text-orange-700">Transfer tepat <strong>{formatPrice(order.total)}</strong> ke BCA 1234 5678 90 a/n Jaya Mandiri.</p>
            </div>
        ...
        {/* Progress Tracker */}
        {['pending', 'verifying', 'reviewing', 'production', 'completed'].includes(order.status) && (
              ...
              <div key={idx} className="flex gap-4 sm:gap-6 pb-6 border-b border-slate-100 last:border-0 last:pb-0">
                ...
                <div className="flex-grow">
                  <div className="flex justify-between items-start mb-1">
                    ...
                    <div className="font-bold text-slate-900">{formatPrice(item.subtotal)}</div>
                  </div>
                  <div className="text-sm text-slate-500 mb-2">{item.quantity} x {formatPrice(item.price)}</div>
