...
export default function LoginPage() {
  ...
  // For Demo Purposes: Allow selecting role
  ...
  const handleLogin = async (e) => {
    ...
    let userData = null;
    if (role === 'customer') {
      userData = { id: 'cust-123', name: 'Budi Santoso', email: email, role: 'customer', phone: '081234567890' };
    } else if (role === 'staff') {
      userData = { id: 'staff-1', name: 'Admin Staff', email: email, role: 'staff' };
    } else if (role === 'manager') {
      userData = { id: 'mgr-1', name: 'Manager Produksi', email: email, role: 'manager' };
    }
        ...
        <div className="bg-white/80 backdrop-blur-xl py-8 px-4 shadow-xl shadow-slate-200/50 sm:rounded-2xl sm:px-10 border border-slate-100">
          <form className="space-y-6" onSubmit={handleLogin}>
            ...
            <div className="p-3 bg-blue-50 border border-blue-100 rounded-xl mb-6">
              <label className="block text-xs font-semibold text-blue-800 uppercase tracking-wider mb-2">Login Sebagai (Demo)</label>
                ...
                {['customer', 'staff', 'manager'].map(r => (
                  <button
                    ...
                    className={`flex-1 py-1.5 text-xs font-medium rounded-md capitalize transition-colors ${
                      role === r ? 'bg-blue-600 text-white' : 'bg-white text-slate-600 border border-slate-200 hover:bg-blue-50'
                    }`}
                  >
                    {r}
                  </button>
