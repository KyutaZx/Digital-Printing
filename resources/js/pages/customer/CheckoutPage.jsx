...
export default function CheckoutPage() {
  ...
  const submitOrder = () => {
    ...
    const newOrder = {
      id: `ORD-${new Date().getFullYear()}${String(new Date().getMonth()+1).padStart(2, '0')}${String(new Date().getDate()).padStart(2, '0')}-${Math.floor(Math.random() * 1000).toString().padStart(3, '0')}`,
      ...
      designFile: form.needsDesign ? null : designFile,
      paymentProof: null,
      createdAt: new Date().toISOString(),
      updatedAt: new Date().toISOString(),
      shippingInfo: form,
      notes: form.needsDesign ? ["Customer request design creation"] : []
    };

    addOrder(newOrder);
    clearCart();
    toast.success('Pesanan berhasil dibuat!');
    navigate(`/pesanan/${newOrder.id}`);
  };
            ...
            {currentStep === 3 && (
              ...
              >
                ...
                <div className="pt-6 flex justify-between border-t border-slate-100">
                  ...
                  <Button onClick={submitOrder} size="lg" className="px-8 shadow-md">
                    Buat Pesanan
                  </Button>
                </div>
              </m.div>
