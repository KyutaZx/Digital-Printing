...
Terlampir di LAMPIRAN Sequence Diagram – *Kelola Produk*

Proses kelola produk dimulai ketika *Owner* memanggil fungsi **fetchProducts()** ke objek `:Products`. Objek `:Products` membalas dengan **data seluruh produk dari tabel Products**. Terdapat percabangan (*alt*) berdasarkan aksi yang dipilih. Jika aksi adalah Tambah Produk, *Owner* mengirim pesan **insertProduct(nama_produk, harga, deskripsi, stok)** ke objek `:Products`, yang merespon dengan **Tampilkan pesan 'Produk berhasil ditambahkan'**. Jika aksi adalah Ubah Data Produk, *Owner* mengirim pesan **updateProduct(product_id, updatedData)** ke objek `:Products`, yang merespon dengan **Tampilkan pesan 'Data produk berhasil diperbarui'**. Jika aksi adalah Hapus Produk, terdapat percabangan (*alt*) validasi tambahan. Jika yakin ingin menghapus bernilai tidak, *Owner* memanggil fungsi **cancelAction()** ke objek `:Products`, dan merespon dengan **Kembali ke daftar produk**. Sedangkan jika yakin ingin menghapus bernilai ya, *Owner* mengirim pesan **deleteProduct(product_id)** ke objek `:Products`, yang kemudian memberikan respon **Tampilkan pesan 'Produk berhasil dihapus'**.
...
Terlampir di LAMPIRAN Sequence Diagram – *Kelola Pengguna*

Proses kelola pengguna dimulai ketika *Owner* memanggil fungsi **fetchUsers()** ke objek `:User`, yang kemudian mengembalikan **data pelanggan & staff dari tabel Users**. Selanjutnya, terdapat percabangan (*alt*) berdasarkan aksi yang dipilih. Jika memilih aksi Tambah Staff, *Owner* mengirim pesan **insertUser(nama, email, password, role)** ke objek `:User`, yang membalas dengan **Tampilkan pesan 'User berhasil ditambahkan'**. Jika memilih aksi Ubah Data / Role, *Owner* mengirim pesan **updateUser(user_id, updatedData)** ke objek `:User`, yang membalas dengan **Tampilkan pesan 'Data user berhasil diperbarui'**. Jika memilih aksi Menonaktifkan Akun, terdapat percabangan (*alt*) konfirmasi lanjutan. Jika tidak yakin menonaktifkan, *Owner* memanggil fungsi **cancelAction()** ke objek `:User`, yang merespon dengan **Kembali ke daftar pengguna**. Sedangkan jika yakin menonaktifkan, *Owner* mengirim pesan **updateStatus(user_id, 'inactive')** ke objek `:User`, yang terakhir memberikan respon **Tampilkan pesan 'Akun berhasil dinonaktifkan'**.
...
Proses kelola material dimulai ketika *Owner* memanggil fungsi **fetchMaterials()** ke objek `:Material`. Objek `:Material` mengembalikan **data daftar material & stok saat ini dari tabel Materials**. Terdapat percabangan (*alt*) berdasarkan pilihan aksi. Jika aksi adalah Tambah Material, *Owner* memanggil **insertMaterial(nama_material, satuan, stok_awal)** ke objek `:Material`. Objek `:Material` lalu memanggil **insertStockLog(material_id, stok_awal, 'in')** ke objek `:MaterialStockLogs`, dan `:Material` merespon dengan **Tampilkan pesan 'Material baru berhasil ditambahkan'**. Jika aksi adalah Ubah Data / Restock, *Owner* memanggil **updateMaterial(material_id, updatedData)** ke objek `:Material`. Di dalamnya terdapat percabangan opsional (*opt*) jika ada penambahan stok, maka `:Material` memanggil **insertStockLog(material_id, qty_tambah, 'in')** ke objek `:MaterialStockLogs`. Setelah itu, `:Material` merespon dengan **Tampilkan pesan 'Data material / stok berhasil diperbarui'**. Terakhir, jika aksi adalah Hapus / Nonaktifkan, terdapat percabangan (*alt*) konfirmasi. Jika tidak yakin menghapus, *Owner* memanggil **cancelAction()** ke objek `:Material`, dan merespon dengan **Batal & Kembali ke daftar material**. Jika yakin menghapus, *Owner* memanggil **updateMaterialStatus(material_id, 'inactive')** ke objek `:Material`, yang kemudian memberikan respon **Tampilkan pesan 'Material dinonaktifkan dari pilihan produksi'**.

---

### 20. Laporan Omzet

Terlampir di LAMPIRAN Sequence Diagram – *Laporan Omzet*

Proses laporan omzet dimulai ketika *Owner* memanggil fungsi **fetchRevenue(startDate, endDate)** ke objek `:PaymentTransaction`. Terdapat percabangan (*alt*) berdasarkan hasil pencarian transaksi. Jika data transaksi tidak ditemukan, objek `:PaymentTransaction` memberikan respon **Tampilkan pesan 'Tidak ada data pendapatan pada periode ini'** kepada *Owner*. Sedangkan jika data transaksi ditemukan, objek `:PaymentTransaction` merespon dengan **data total omzet (status 'approved')** kepada *Owner*. Setelah itu, *Owner* memanggil fungsi **fetchOrderDetails(periode_tersebut)** ke objek `:Orders`, yang kemudian memberikan respon **Tampilkan grafik omzet dan tabel detail pesanan** kepada *Owner*. Di dalam kondisi ini, terdapat percabangan (*alt*) lanjutan terkait pengunduhan laporan. Jika memilih untuk tidak mengunduh laporan, maka tidak ada aksi lanjutan. Namun jika memilih untuk mengunduh laporan, *Owner* memanggil fungsi **exportReport(format)** ke objek `:PaymentTransaction`, yang kemudian memberikan respon **Mengunduh file laporan berformat PDF / Excel** kepada *Owner*.
